...
if __name__ == '__main__':
    def test_case_1():
        ...
        test_union = union(linked_list_1, linked_list_2)
        answer = []
        node = test_union.head
        while node:
            answer.append(node.value)
            node = node.next
        assert answer == [1, 2, 3, 4, 5, 6, 7]

        print("Instersection for linked lists containing [1, 2, 3, 4, 5] and [3, 4, 5, 6, 7]")
        # Solution: 3 -> 4 -> 5 ->
        test_intersection = intersection(linked_list_1, linked_list_2)
        answer = []
    ...
    def test_case_2():
        ...
        while node:
            answer.append(node.value)
            node = node.next
        assert answer == [3, 2, 4, 35, 6, 65, 21, 32, 9, 1, 11]
    ...
    def test_case_4():
        ...
        while node:
            answer.append(node.value)
            node = node.next
        assert answer == ['x', None, 'z', 1, 3, 'j', 'k', 2]
